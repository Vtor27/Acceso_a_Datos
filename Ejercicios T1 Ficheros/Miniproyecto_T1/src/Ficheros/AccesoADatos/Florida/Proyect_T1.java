...
public class Proyect_T1 {
	public static void renombrar(String carpeta) {
		Scanner scanner = new Scanner(System.in); 
		mostrarContenido(carpeta);
		
		File file = new File(carpeta);
		String[] contenidoCarpeta = file.list(); 
		System.out.println("Selecciona el número del fichero/carpeta que desee renombrar. --> ");
		
		int num = scanner.nextInt();
		File elementoRenombrar = new File(contenidoCarpeta[num - 1]);
		System.out.println("Elige el nuevo nombre.");
		String nuevoNombre = scanner.next();
		File newFile = new File(nuevoNombre);
		boolean exito = elementoRenombrar.renameTo(newFile);
		if(exito) {
			System.out.println("El nombre ha sido cambiado con exito.");
		}else {
			System.out.println("Error al intentar cambiar en nombre.");
		}
		mostrarContenido(carpeta);
